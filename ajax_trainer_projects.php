<?php
	...
	$page_start 		= $start +1;
	$projects			= $db->get_trainer_projects($start, PER_PAGE, $searchval,$sort, $order);
	$paginationCount 	= $db->get_trainer_project_count( $searchval);
?>
			<table class="table table-striped table-bordered table-hover" 
                data-provide="datatable"  >
                  <thead>
                    <tr>
                    	<th onclick="changeValue('sort','username');" class="<?php
					  echo ($sort == 'username')? 
					   "sorting_".$order : "sorting"; ?>">Participant Name</th>
					   <th>Participant Creator </th>
