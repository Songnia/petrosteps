<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' && @$_POST['action'] == 'create_project') {
		...
		if($projectId){
			...
			if($db->check_project_exists($_POST['new_project'], $_SESSION['user_id']))
				$_SESSION['message'] = 'Error in adding project. Project with same name already exists';
			else
				$_SESSION['message'] = 'Error in adding project. Please try again';
...
<head>
  ...
  <title>Project Management — <?php echo APP_NAME; ?></title>
...
<?php
  if(isset($_SESSION['user_type']) && in_array($_SESSION['user_type'], array('Admin', 'SuperAdmin', 'Trainer'))) {
...
<div class="user-content-wrapper">
  ...
  <!-- PAGE HEADER -->
  <div class="md3-page-header" style="margin-bottom:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
    <div>
      <h1 class="md3-page-title">Project Management</h1>
      <p class="md3-page-subtitle">
        Welcome back, <strong><?php echo htmlspecialchars($_SESSION['username']); ?></strong>
        — create a new project or select an existing simulation
      </p>
    </div>
    <?php if(isset($_SESSION['user_type']) && $_SESSION['user_type'] !== 'Participant'): ?>
    <div>
      <a href="ongoing_project_list.php" class="md3-btn md3-btn--outline">
...
<?php
	if(isset($_SESSION['message'])) {
		echo '<div class="md3-alert md3-alert--'.$_SESSION['message_type'].'" style="margin-bottom:20px;">
...
?>
  ...
  <div class="user-section">
    ...
    <div class="md3-table-container table-responsive-wrapper">
