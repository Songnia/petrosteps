...
/*-- END LISTS --*/
	...
	if ($inherit == 'BLOCK') {
		...
		// Save Cascading CSS e.g. "div.topic p" at this block level
			...
			//===============================================
			$parentCascadeCSS = array();
			if (isset($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS']) && is_array($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS'])) {
				$parentCascadeCSS = $this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS'];
			}
			$this->_mergeFullCSS($parentCascadeCSS, $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes, $attr['ID']);
		//===============================================
		  // Block properties
