...
$db = new DB();
  ...
  if($_SESSION['session_id'] !== $session_id){
    ...
    $pass_change = $db->change_password($current, $confirm_pwd, $user_id, $username, false);
		...
		$_SESSION['message_type'] = 'danger';
		$_SESSION['message'] = 'Error in updaing Trainer, Entry already exists';
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action'] == 'delete_user'){
		...
		$_SESSION['message_type'] = 'danger';
		$_SESSION['message'] = 'Error in deleting Trainer please try again';
		if($delete)
		{
			$_SESSION['message_type'] = 'success';
...
<head>
  ...
  <title>Edit Trainer — <?php echo APP_NAME; ?></title>
...
<div class="md3-page-header">
  <div>
    <h1 class="md3-page-title">Edit Trainer</h1>
    <p class="md3-page-subtitle">Update trainer information</p>
  </div>
</div>
...
<div class="md3-form-card" style="max-width:600px;">
  ...
  <div class="md3-form-card__body">
    <form id="validate-basic" action="" data-validate="parsley" class="form parsley-form" method="post">
      ...
      <div class="md3-field">
        <input type="text" id="firstname" name="firstname" required placeholder=" " data-required="true" value="<?php echo $user['firstname'];?>">
        <label for="firstname">Name</label>
      </div>

      <div class="md3-field">
        <input type="text" id="email" name="email" required placeholder=" " data-required="true" value="<?php echo $user['email'];?>">
      ...
      <div class="md3-form-card__actions">
        <a href="trainer_list.php" class="md3-btn md3-btn--outlined">Cancel</a>
